...
The groups.regex should contain regular expressions to match sample names. 
The order of groups.regex matters, such that the first match will determine the grouping;
this is why the "case" group corresponds to an empty string, that matches anything, hence
...
## ECDF plotting
We can plot an empirical distribution function (ECDF) of repeat scores for the
Huntington disease (HD) locus.
...
For convience and speed in this vignette, we only assess expansions for the
Huntington disease, spinocerebellar ataxias 2 and 6, and Friedrich's ataxia loci. 
```{r}
( str_score_four <- str_score[c("HD", "SCA2", "SCA6", "FRDA")] )
```

For creating multiple ECDF curves to files, you may use the `plot_multi()` function. 
...
```{r, out.width = 6, out.height = 6, fig.width=12, fig.height=12}
par(mfrow = c(2, 2))
...
You may manually set the colours each sample will use with a vector of colors, with names
