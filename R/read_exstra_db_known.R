...
read_exstra_db_known <- function(file, ...) {
  ...
  testit::assert("Known requires Disease or locus column", ! is.null(data$Disease) || ! is.null(data$locus))
  if(is.null(data$Disease)) {
    testit::assert("Known requires Disease or locus column", ! is.null(data$locus))
